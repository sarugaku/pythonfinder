...
class WindowsFinder(BaseFinder):
    ...
    def find_python_version(self, major, minor=None, patch=None, pre=None, dev=None):
        version_matcher = operator.methodcaller('matches', major, minor=minor, patch=patch, pre=pre, dev=dev)
        py_filter = filter(None, filter(lambda c: version_matcher(c), self.version_list))
    ...
    def get_versions(self):
        ...
        for version_object in env_versions:
            ...
            base_dir = PathEntry.create(path, is_root=True, only_python=True, pythons={
                py_version.comes_from.path: py_version
            })
