...
class Finder(object):
    ...
    def system_path(self):
        if not self._system_path:
            self._system_path = SystemPath.create(path=self.path_prepend, system=self.system)
    ...
    def find_python_version(self, major, minor=None, patch=None, pre=False, dev=False):
        ...
        if os.name == 'nt':
            match = self.windows_finder.find_python_version(major, minor=minor, patch=patch, pre=pre, dev=dev)
            if match:
                return match
        return self.system_path.find_python_version(major, minor=minor, patch=patch, pre=pre, dev=dev)
